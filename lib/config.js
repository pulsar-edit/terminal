        ...
        behavior: {
            ...
            properties: {
                ...
                requireModifierToOpenUrls: {
                    ...
                    description: `When enabled, you must hold down ${utils.isMac() ? '`Cmd`' : '`Ctrl`'} while clicking on a URL — or a detected local file path — in order to open it.`,
                    type: 'boolean',
                    default: true,
                    order: 8
                },
...
async function setAutoShell() {
    ...
    if (!command)
        return;
    atom.config.set(`${utils.PACKAGE_NAME}.terminal.shell`, command);
}
async function possiblySetAutoShell() {
    if (localStorage.getItem(`${utils.PACKAGE_NAME}.autoShellSet`) !== null) {
        return;
    }
    ...
    // bite at the apple.
    localStorage.setItem(`${utils.PACKAGE_NAME}.autoShellSet`, 'true');
