...
const SCRIPT_ROOT = path__namespace.join(PACKAGE_ROOT, 'shell');
...
function arePowerShellLoginArgs(args) {
    if (isString(args))
        return POWERSHELL_LOGIN_ARGS.includes(args.toLowerCase());
    if (args.length === 1)
        return POWERSHELL_LOGIN_ARGS.includes(args[0].toLowerCase());
    if (args.length === 2) {
...
function arePowerShellImpliedArgs(args) {
    if (isString(args))
        return POWERSHELL_IMPLIED_ARGS.includes(args.toLowerCase());
    if (args.length === 0)
        return true;
    if (args.length === 1)
        return POWERSHELL_IMPLIED_ARGS.includes(args[0].toLowerCase());
    return false;
}
...
function safeUsername() {
    try {
        return os__namespace.userInfo().username;
    }
    catch {
        return 'unknown';
    }
...
function realTempDir() {
    try {
        return fs__default.default.realpathSync(os__namespace.tmpdir());
    }
    catch {
...
async function getShellIntegrationInjection(shellCommand, shellArgs, env) {
    ...
    if (utils.isWindows() && (utils.windowsBuildNumber() ?? 0) < MINIMUM_WINDOWS_BUILD_FOR_SHELL_INTEGRATION) {
        return { enabled: false, reason: 'Windows build too old' };
    }
    let shell = path__namespace.basename(shellCommand).toLowerCase();
    let envMixin = {
    ...
    switch (shell) {
        ...
        default:
            log.debug(`Shell integration: unsupported shell "${shell}"`);
...
function injectBash(originalArgs, envMixin) {
    ...
    let args = ['--init-file', path__namespace.join(SCRIPT_ROOT, 'shell-integration-bash.sh')];
...
function injectFish(originalArgs, envMixin) {
    ...
    let args = [
        ...(isLogin ? ['-l'] : []),
        '--init-command',
        `source "${path__namespace.join(SCRIPT_ROOT, 'shell-integration.fish')}"`
    ];
    return { enabled: true, injection: { args, env: envMixin } };
}
function injectPowerShell(originalArgs, envMixin) {
    ...
    let scriptPath = path__namespace.join(SCRIPT_ROOT, 'shell-integration.ps1');
    ...
    let sourceCommand = utils.isWindows() ?
...
async function injectZsh(originalArgs, envMixin, env) {
    ...
    let zdotdir = path__namespace.join(realTempDir(), `${safeUsername()}-pulsar-zsh`);
    try {
        await fs__default.default.ensureDir(zdotdir);
        ...
        await fs__default.default.chmod(zdotdir, 0o1700);
    }
    catch (err) {
        log.error(`Shell integration: failed to prepare ZDOTDIR at ${zdotdir}: ${err}`);
    ...
    await Promise.all([
        fs__default.default.copy(path__namespace.join(SCRIPT_ROOT, 'shell-integration-rc.zsh'), path__namespace.join(zdotdir, '.zshrc'), copyOptions),
        fs__default.default.copy(path__namespace.join(SCRIPT_ROOT, 'shell-integration-profile.zsh'), path__namespace.join(zdotdir, '.zprofile'), copyOptions),
        fs__default.default.copy(path__namespace.join(SCRIPT_ROOT, 'shell-integration-env.zsh'), path__namespace.join(zdotdir, '.zshenv'), copyOptions),
        fs__default.default.copy(path__namespace.join(SCRIPT_ROOT, 'shell-integration-login.zsh'), path__namespace.join(zdotdir, '.zlogin'), copyOptions)
    ]);
    envMixin.ZDOTDIR = zdotdir;
    envMixin.USER_ZDOTDIR = env.ZDOTDIR ?? os__namespace.homedir() ?? '~';
    return { enabled: true, injection: { args, env: envMixin } };
}
