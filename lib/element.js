...
class TerminalElement extends HTMLElement {
    ...
    activateLink(event, uri, _range) {
        ...
        if (shouldOpenInPulsar) {
            ...
            remote.shell.showItemInFolder(uri);
        }
    }
    hoverLink(_event, uri, range, _rangeType) {
        if (!this.terminal)
            return;
        ...
        if (range) {
            if (this.rangesAreEqual(range, this.tooltipRange) && this.tooltip && this.tooltipHideTimer) {
                ...
                log.log('Keeping existing tooltip!', this.inspectRange(range));
                ...
                log.warn('Making new tooltip for range', this.inspectRange(range), reason, `(Last range: ${this.inspectRange(this.tooltipRange)})`);
                ...
                baseY } = this.cursorPosition ?? { cursorY: 0, baseY: 0 };
    ...
    leaveLink(event, uri, range) {
        log.debug('#leave', uri, event, range);
    ...
    addTooltip(element, tooltipText) {
        ...
        let trigger = range.start.y === range.end.y ? 'hover' : 'manual';
