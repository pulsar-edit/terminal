...
var index = require('./shell-integration/index.js');
var addon = require('./shell-integration/addon.js');
var provider = require('./link-detection/provider.js');

function _interopDefault (e) { return e && e.__esModule ? e : { default: e }; }

var fs__default = /*#__PURE__*/_interopDefault(fs);

var _DelayedPresence_instances, _DelayedPresence_inState, _DelayedPresence_isSame, _DelayedPresence_reset, _DelayedPresence_effectiveShowDelay, _DelayedPresence_cancelTimer, _TerminalElement_instances, _TerminalElement_mainResizeObserver, _TerminalElement_mainContentRect, _TerminalElement_terminalIntersectionObserver, _TerminalElement_terminalInitiallyVisible, _TerminalElement_fitAddon, _TerminalElement_searchAddon, _TerminalElement_shellIntegrationAddon, _TerminalElement_webglAddon, _TerminalElement_prioritizedPrefixes, _TerminalElement_ptyMeta, _TerminalElement_loseWebglContext, _TerminalElement_bindingMatchesAncestorChain, _TerminalElement_shouldPrioritizeBinding, _TerminalElement_keymapHasPendingPartialMatches, _TerminalElement_keyboardEventMatchesKeybinding, _TerminalElement_buildAncestorChain, _TerminalElement_clipboardBindingForKeyboardEvent, _TerminalElement_createTerminal, _TerminalElement_restartPtyProcess;
/**
 ...
 */
const CONTINGENT_ENV_DENY_LIST = {
...
class TerminalElement extends HTMLElement {
    ...
    constructor() {
        ...
        this.warnAboutClipboardKeybindings = false;
        ...
        _TerminalElement_shellIntegrationAddon.set(this, void 0);
    ...
    async initialize(model) {
        ...
        try {
            ...
            }), atom.config.observe('terminal.advanced.warnAboutClipboardKeybindings', (newValue) => {
    ...
    activateLink(event, uri, _range) {
        ...
        const behavior = config.Config.get('behavior.localPathBehavior');
    ...
    optionallyWarnAboutModifierlessClick() {
        ...
        atom.notifications.addInfo(`Terminal: Click ignored`, {
            description: `For security and protection against accidental clicks, you must hold <kbd>${utils.isMac() ? 'Cmd' : 'Ctrl'}</kbd> while clicking URLs in order to open them in your browser. You may disable this requirement in the package settings.\n\n**(This message will be shown only once.)**`,
            dismissable: true,
            buttons: [
                {
    ...
    optionallyWarnAboutClipboardKeybindings(binding) {
        if (!config.Config.get('advanced.warnAboutClipboardKeybindings')) {
            return;
        }
        let { keystrokes, command } = binding;
        config.Config.set('advanced.warnAboutClipboardKeybindings', false);
        let list = this.buildKeybindingListForClipboardActions();
        atom.notifications.addInfo(`Terminal: Clipboard keybinding inactive`, {
            description: `The keystroke ${utils.keystrokeToHTML(keystrokes)} is ordinarily bound to the \`${command}\` clipboard command, but has special meaning when the terminal has focus. ${list}\n\n**(This message will be shown only once.)**`,
            dismissable: true,
            buttons: [
                {
                    text: 'Open Terminal Settings',
    ...
    buildKeybindingListForClipboardActions() {
        ...
        for (let command of CLIPBOARD_COMMANDS) {
            ...
            if (!binding)
                continue;
            listItems.push(`* \`${command}\`: ${utils.keystrokeToHTML(binding.keystrokes)}`);
        }
    ...
    keyBindingForCommand(command, ownBindingsOnly = false) {
        let bindings = atom.keymaps.findKeyBindings({ command, target: this });
        bindings = bindings.filter(kb => kb.enabled !== false);
        if (ownBindingsOnly) {
            let packagePath = atom.packages.getLoadedPackage(utils.PACKAGE_NAME)?.path;
            if (!packagePath)
                return null;
     ...
     * provider.
     */
    activateLocalPathLink(event, targetPath, isDirectory, line, column) {
        if (config.Config.get('behavior.requireModifierToOpenUrls')) {
            let modifier = utils.isMac() ? event.metaKey : event.ctrlKey;
            if (!modifier) {
                this.optionallyWarnAboutModifierlessClick();
    ...
    async restartPtyProcess() {
        ...
        this.restartingPromise = tslib.__classPrivateFieldGet(this, _TerminalElement_instances, "m", _TerminalElement_restartPtyProcess).call(this);
...
_TerminalElement_mainResizeObserver = new WeakMap(), _TerminalElement_mainContentRect = new WeakMap(), _TerminalElement_terminalIntersectionObserver = new WeakMap(), _TerminalElement_terminalInitiallyVisible = new WeakMap(), _TerminalElement_fitAddon = new WeakMap(), _TerminalElement_searchAddon = new WeakMap(), _TerminalElement_shellIntegrationAddon = new WeakMap(), _TerminalElement_webglAddon = new WeakMap(), _TerminalElement_prioritizedPrefixes = new WeakMap(), _TerminalElement_ptyMeta = new WeakMap(), _TerminalElement_instances = new WeakSet(), _TerminalElement_loseWebglContext = function _TerminalElement_loseWebglContext() {
...
}, _TerminalElement_bindingMatchesAncestorChain = function _TerminalElement_bindingMatchesAncestorChain(kb, ancestorChain) {
    ...
    if (ancestorChain) {
        ...
        if (!tslib.__classPrivateFieldGet(this, _TerminalElement_instances, "m", _TerminalElement_bindingMatchesAncestorChain).call(this, kb, ancestorChain))
            return false;
    ...
    if (!target)
        return false;
    let ancestorChain = tslib.__classPrivateFieldGet(this, _TerminalElement_instances, "m", _TerminalElement_buildAncestorChain).call(this, target);
    ...
    return result;
}, _TerminalElement_buildAncestorChain = function _TerminalElement_buildAncestorChain(node) {
    let results = [];
    ...
    return results;
}, _TerminalElement_clipboardBindingForKeyboardEvent = function _TerminalElement_clipboardBindingForKeyboardEvent(event) {
    let keystroke = atom.keymaps.keystrokeForKeyboardEvent(event);
    // @ts-ignore Undocumented.
    let bindings = atom.keymaps.findMatchCandidates([keystroke], []);
    let ancestorChain = tslib.__classPrivateFieldGet(this, _TerminalElement_instances, "m", _TerminalElement_buildAncestorChain).call(this, this);
    return bindings.exactMatchCandidates.find((kb) => {
        if (!tslib.__classPrivateFieldGet(this, _TerminalElement_instances, "m", _TerminalElement_bindingMatchesAncestorChain).call(this, kb, ancestorChain)) {
            return false;
        }
    ...
    this.terminal.onKey((event) => {
        ...
        if (this.warnAboutClipboardKeybindings &&
            ...
            let clipboardBinding = tslib.__classPrivateFieldGet(this, _TerminalElement_instances, "m", _TerminalElement_clipboardBindingForKeyboardEvent).call(this, event.domEvent);
            if (clipboardBinding) {
    ...
    if (config.Config.get('terminal.enableShellIntegration')) {
        tslib.__classPrivateFieldSet(this, _TerminalElement_shellIntegrationAddon, new addon.ShellIntegrationAddon(), "f");
        this.terminal.loadAddon(tslib.__classPrivateFieldGet(this, _TerminalElement_shellIntegrationAddon, "f"));
        this.subscriptions.add(tslib.__classPrivateFieldGet(this, _TerminalElement_shellIntegrationAddon, "f").onDidChangeCwd((cwd) => {
            log.debug('Shell integration: cwd changed:', cwd);
            if (this.model)
                this.model.setCwd(cwd);
        }), tslib.__classPrivateFieldGet(this, _TerminalElement_shellIntegrationAddon, "f").onDidExecuteCommand((command) => {
            log.debug('Shell integration: command executing:', command);
        }), tslib.__classPrivateFieldGet(this, _TerminalElement_shellIntegrationAddon, "f").onDidFinishCommand((command) => {
            log.debug('Shell integration: command finished:', command);
        }));
    }
    if (config.Config.get('xterm.localPathDetection')) {
        this.terminal.registerLinkProvider(new provider.LocalPathLinkProvider(this.terminal, () => this.model?.getPath(), (event, targetPath, isDirectory, line, column) => this.activateLocalPathLink(event, targetPath, isDirectory, line, column)));
    }
...
}, _TerminalElement_restartPtyProcess = async function _TerminalElement_restartPtyProcess() {
    ...
    let env = this.getEnv();
    let result = await index.getShellIntegrationInjection(command, args, env);
    if (result.enabled) {
        log.debug('Shell integration injected:', result.injection);
        let injection = result.injection;
        env = { ...env, ...injection.env };
        args = injection.args;
        tslib.__classPrivateFieldGet(this, _TerminalElement_shellIntegrationAddon, "f")?.setNonce(injection.env.PULSAR_TERMINAL_NONCE);
    }
    else {
        log.debug('Shell integration not injected:', result.reason);
        tslib.__classPrivateFieldGet(this, _TerminalElement_shellIntegrationAddon, "f")?.setNonce(undefined);
    }
    tslib.__classPrivateFieldGet(this, _TerminalElement_ptyMeta, "f").options ??= {};
    tslib.__classPrivateFieldGet(this, _TerminalElement_ptyMeta, "f").command = command;
    tslib.__classPrivateFieldGet(this, _TerminalElement_ptyMeta, "f").args = args;
