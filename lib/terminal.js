...
class Terminal {
    static activate(_state) {
        ...
        element.registerTerminalElement();
        log.initialize();
        ...
        atom.views.addViewProvider(model.TerminalModel, (model) => {
            ...
            let element$1 = element.TerminalElement.create();
        ...
        }), atom.commands.add(`[${element.TERMINAL_ELEMENT_ATTRIBUTE}]`, {
            'core:copy': (event) => {
    ...
    static describeFocus(target) {
        ...
        if (target.closest(`[${element.TERMINAL_ELEMENT_ATTRIBUTE}]`))
            return null;
    ...
    static inferTerminalElement(event) {
        if (!event.target || !(event.target instanceof HTMLElement))
            return null;
        return event.target.closest(`[${element.TERMINAL_ELEMENT_ATTRIBUTE}]`);
    }
    static async open(uri, rawOptions = {}) {
        let url = new URL(uri);
        ...
        log.debug('Opening terminal with options:', options, url.href);
        try {
            let item = await atom.workspace.open(url.href, options);
            if (!item)
    ...
    // item in the current pane instead. (Compare
    // `Pane::findOrCreateBottommostSibling` to `Pane::findTopmostSibling` in the
    // Pulsar source.)
    //
    ...
    // one.
    static createPaneForSplit(options) {
        if (options.pane)
            return undefined;
        if (options.split !== 'up' && options.split !== 'left')
            return undefined;
        let container = this.getWorkspaceLocationContainer(options.location);
        let pane = container?.getActivePane();
        if (!pane)
            return undefined;
    ...
    static destroyPaneIfEmpty(pane) {
        if (!pane || pane.getItems().length > 0)
            return;
        pane.destroy();
    }
    static getWorkspaceLocationContainer(location) {
        switch (location) {
            case 'center':
