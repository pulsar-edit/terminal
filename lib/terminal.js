...
class Terminal {
    static activate(_state) {
        ...
        atom.config.onDidChange('terminal.appearance', debouncedUpdateTheme), atom.themes.onDidChangeActiveThemes(debouncedUpdateTheme));
        let docks = [
    ...
    static inferTerminalElement(event) {
        if (!event.target || !(event.target instanceof HTMLElement))
            return null;
        return event.target.closest('pulsar-terminal');
    }
    ...
    static performOnActiveTerminal(operation) {
